...
$proxyServerRegQuery="insert into proxy_server_info VALUES (DEFAULT,INET_ATON('$hostIp'),'$hostLocation','$serverName','$serverPort','$webUrl','$pingService','$portService','$httpsService','$pauseOrStart','$normalMinutes','$problemMinutes','$recheckTime')";
if ($con->query($proxyServerRegQuery) === TRUE ) {
  $serverId = mysqli_insert_id($con);
  $pingServiceTable=$serverId . "_proxy_Server_Ping_Service";
$portServiceTable=$serverId . "_proxy_Server_Port_Service";
$httpsRequestServiceTable=$serverId . "_proxy_Server_https_Request_Service";
...
curl_close($curl);
    echo "<script type=\"text/javascript\">window.alert('Your Proxy server has been successfully added.');
window.location.href = 'add_New_Server.php';</script>";
exit;

} else {
    echo "Error: " . $proxyServerRegQuery . "<br>" . $con->error;
}
$con->close();
?>