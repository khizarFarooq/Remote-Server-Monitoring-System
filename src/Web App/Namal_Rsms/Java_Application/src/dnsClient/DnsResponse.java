...
public class DnsResponse {
    ...
    private void checkRCodeForErrors() {
        switch (this.RCode) {
            case 0:
                ...
                break;
            case 1:
                DnsClient.status = "Format error";
               // throw new RuntimeException("Format error: the name server.server was unable to interpret the query");
            case 2:
                DnsClient.status = "Server failure";
               // throw new RuntimeException("Server failure: the name server.server was unable to process this query due to a problem with the name server.server");
            case 3:
                DnsClient.status = "Name Error";
                //throw new MissingDomainException();
            case 4:
                DnsClient.status = "Not implemented";
                //throw new RuntimeException("Not implemented: the name server.server does not support the requested kind of query");
            case 5:
                DnsClient.status = "Refused due to policy reasons";
                //throw new RuntimeException("Refused: the name server.server refuses to perform the requested operation for policy reasons");
        }
    }
    ...
    private void validateQueryTypeIsResponse() {
        if (!this.QR) {
            throw new RuntimeException("ERROR\tInvalid response from server.server: Message is not a response");
        }
    }
